...
// Fantasy scoring rules - base values (PPR is applied per contest type)
  ...
  try {
    ...
    if (finalGames.length > 0) {
      ...
      for (const game of finalGames) {
        ...
        const existingPlayerIds = new Set(existingStats?.map(s => s.player_card_id) || [])
        
        // Create 0-point entries for players without stats
        const playersWithoutStats = teamPlayers.filter(p => !existingPlayerIds.has(p.id))
        
        if (playersWithoutStats.length > 0) {
          console.log(`Creating ${playersWithoutStats.length} zero-stat entries for game ${gameId}`)
          
          const zeroStatEntries = playersWithoutStats.map(p => ({
            game_id: gameId,
            player_card_id: p.id,
            week_number: weekNumber,
            season_year: seasonYear,
    ...
    const tokenCardsMap = new Map(
      allTokenCards?.map(t => [t.id, t]) || []
    )

    // Step 4: Calculate and update lineup totals
    ...
    for (const lineup of lineups) {
      ...
      const { data: playerStats } = await supabase
        .from('player_game_stats')
        .select('player_card_id, fantasy_points, stats')
        .in('player_card_id', playerCardIds)
        .eq('week_number', weekNumber)
        .eq('season_year', seasonYear)
      
      const statsMap = new Map(
        playerStats?.map(s => [s.player_card_id, { fantasy_points: s.fantasy_points, stats: s.stats }]) || []
      )
      ...
      const inventoryMap = new Map(
        inventory?.map(i => [i.player_card_id, i]) || []
      )
      ...
      const tokensMap = new Map(
        appliedTokens?.map(t => [t.applied_to_player_id, t]) || []
      )
      ...
      for (const [position, playerData] of Object.entries(snapshot) as any) {
        ...
        const playerStatsData = statsMap.get(playerData.player_card_id)
        const basePoints = playerStatsData?.fantasy_points || 0
        const rawStats = playerStatsData?.stats || {}
    ...
    console.log(`📊 Updated ${lineupsUpdated} lineup scores`)
